...
bool AflTraceMap::isCrackBranch(UINT16 edge) {
  bool ret = true;
  crack_virgin_map_[edge]++;
  if ((crack_virgin_map_[edge] | crack_map_[edge]) != crack_map_[edge]) {
    
    // TODO: delete the log info
    printf("[DEBUG]: Start to crack branch %d\n", edge);

    crack_map_[edge] |= crack_virgin_map_[edge];
  }
  else
    ret = false;
